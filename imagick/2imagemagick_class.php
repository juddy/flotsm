		...
		var $verbose        = FALSE;
			...
			*/
			...
			function Flotsm($format='jpg') {
				...
				$command = "{$this->imagemagickdir}/convert -colorspace RGB -draw 'roundRectangle  10,10 60,80 5,5' -draw 'text 20,20 made @ flotsm.com' -quality {$this->jpg_quality} '{$this->temp_dir}/tmp{$this->count}_{$this->temp_file}' '{$this->temp_dir}/tmp".++$this->count."_{$name}'";
				...
				if($returnvalue) {
					$this->error .= "ImageMagick: Flotsm failed\n";
					if($this->verbose == TRUE) {
						echo "Convert failed\n";
					}
				} else {
			...
			*/
			...
			function Convert($format='jpg') {
				...
				$command = "{$this->imagemagickdir}/convert -colorspace RGB -draw 'text 10,10 flotsm.com' -quality {$this->jpg_quality} '{$this->temp_dir}/tmp{$this->count}_{$this->temp_file}' '{$this->temp_dir}/tmp".++$this->count."_{$name}'";
